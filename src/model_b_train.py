...
def build_distractor_features(df: pd.DataFrame, vectorizer) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        # Positive examples — wrong options from RACE (good distractors)
        for lbl in ["A", "B", "C", "D"]:
            if lbl == row["answer"]:
                continue
        ...
        # Negative examples — random short sentences from article (bad distractors)
        for sent in sentences[:2]:
            if correct_text.lower()[:20] in sent.lower():
                continue
            v_s = vectorizer.transform([sent])
            v_a = vectorizer.transform([article_text])
            v_c = vectorizer.transform([correct_text])
            rows.append({
                "sim_to_article": cosine_similarity(v_s, v_a)[0][0],
                "sim_to_correct": cosine_similarity(v_s, v_c)[0][0],
                "len_ratio":      len(sent.split()) / max(1, len(correct_text.split())),
                "label":          0,
            })
...
def train_hint_scorer(train_df: pd.DataFrame, vectorizer) -> LogisticRegression:
    ...
    for _, row in sample.iterrows():
        ...
        # Split on periods and filter — lowered threshold to 5 chars
        ...
        for i, (sent, sim) in enumerate(zip(sents, sims)):
            ...
            feats = [
                sim,
                cosine_similarity(vectorizer.transform([sent]),
                                  vectorizer.transform([article]))[0][0],
                len(sent.split()),
                i / max(1, len(sents)),
                len(set(sent.lower().split()) & set(question.lower().split())),
            ]
            rows.append(feats + [label])

    print(f"  Collected {len(rows)} hint training samples")

    if len(rows) == 0:
        print("  ⚠ No samples collected — saving dummy scorer")
        from sklearn.dummy import DummyClassifier
        scorer = DummyClassifier(strategy="most_frequent")
        scorer.fit([[0,0,0,0,0]], [1])
